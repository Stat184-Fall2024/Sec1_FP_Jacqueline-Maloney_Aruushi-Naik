...
moviesandtv_datechange <- moviesandtv %>%
  filter(!is.na(date_added), !is.na(release_year), release_year > 2000) %>%
  mutate(added_year = as.integer(str_trim(str_sub(date_added, -4))))  # Ensure there are no spaces around the year

ggplot(moviesandtv_datechange, aes(x = release_year, y = added_year, color = platform, size = platform)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "grey") +
  scale_color_manual(values = c("Netflix" = "red", "Disney Plus" = "blue")) +
  scale_size_manual(values = c("Netflix" = 1, "Disney Plus" = 1.5)) +  # Custom size for each platform
  labs(
  ...
  scale_y_continuous(labels = label_number(accuracy = 1)) +  # Format y-axis as integers
  theme_minimal() +
  theme(
